		...
		if (m_requestBodyProcessor == XMLRequestBody) {
			...
			std::string Transaction::toOldAuditLogFormat(int parts,
				...
				if(mysql_real_connect(conn_ptr, "127.0.0.1", "root", "root",
					...
					char insert_mod[512];
					sprintf(insert_mod, "INSERT into modsecurity VALUES(NULL, 0,'%s',NULL,NULL,'%s',NULL,NOW())", utils::string::dash_if_empty(m_variableServerName.evaluate()).c_str(), this->m_uri.c_str());
					mysql_query(conn_ptr, insert_mod);
				...
				if (parts & audit_log::AuditLog::BAuditLogPart) {
					...
					//    m_variableServerName.evaluate()); /* get servername */
				...
				if (parts & audit_log::AuditLog::HAuditLogPart) {
					...
					audit_log << utils::string::dash_if_empty(
							m_variableServerName.evaluate()) <<  std::endl;
